<template>
  <div style="display:flex;flex-direction:column">
    <div style="margin-top:50px;margin-left:15px;font-size:25px">找回密码</div>
     <van-field style="margin-top:50px" v-model="value" type="tel" placeholder="请输入手机号码">
       <template #label>
     ...
     <van-field v-model="value" center clearable label="短信验证码" placeholder="请输入短信验证码">
      <template #button>
        <van-button size="small" type="info" plain>发送验证码</van-button>
      </template>
     </van-field>
     <van-field v-model="value" label="新密码" placeholder="请输入密码" />
     <van-field v-model="value" label="确认密码" placeholder="请再次确认密码" />
     <van-button  style="width:80%;margin-top:30px;align-self:center;" type="info" @click="submit">确定</van-button>
